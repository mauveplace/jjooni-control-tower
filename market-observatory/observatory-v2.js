...
function openFullscreen(id){
  ...
  const o=ensureFullscreen(),range=active[id]||'5Y',f=filtered(r.labels,r.sets,range);
...
function eventMetricsHtml(e){
  ...
  if(ms.length)return `<div class="obsMetricTable">${ms.map(m=>`<div class="obsMetricRow"><div class="obsMetricName">${esc(m.label)}</div><div class="obsPrev">이전 <b>${esc(val(m.previous))}</b></div><div class="forecast">예측 <b>${esc(val(m.consensus))}</b></div><div class="actual">실제 <b>${esc(val(m.actual))}</b></div></div>`).join('')}</div>`;
...
window.renderCalendar=function(){
  ...
  box.innerHTML=xs.length?xs.map(e=>{
    const dt=(e.datetime_kst||e.date||'').slice(5,16).replace('T',' '),hasActual=(e.market_metrics||[]).some(m=>m?.actual!=null)||e.actual!=null,status=hasActual?'<span class="obsDone">발표완료</span>':'<span class="obsPending">예정</span>';
...
.obsRange{align-items:center;margin:8px 0 2px;overflow-x:auto;flex-wrap:nowrap;padding-bottom:5px;-webkit-overflow-scrolling:touch;scrollbar-width:none}.obsRange::-webkit-scrollbar{display:none}.obsRange button{white-space:nowrap;flex:0 0 auto;min-height:30px}.obsFullBtn{font-weight:900!important}.obsGesture{align-self:center;white-space:nowrap}.obsCoverage{display:block;margin:2px 0 5px;color:#718096;font-size:9px;line-height:1.35;font-weight:650}.obsEventMetrics{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px;font-size:10px}.obsEventMetrics span{background:#f6f8fb;border:1px solid #e7ecf2;padding:5px 7px;border-radius:7px;min-width:0}.obsEventMetrics b{margin-left:3px}.event{align-items:start}.obsNoScroll{overflow:hidden!important}.obsDone{color:#087443;font-weight:900}.obsPending{color:#718096}.obsMetricTable{margin-top:7px;border:1px solid #e7ecf2;border-radius:10px;overflow:hidden;width:100%;max-width:680px}.obsMetricRow{display:grid;grid-template-columns:minmax(120px,1.35fr) repeat(3,minmax(72px,.8fr));background:#fff;font-size:10px;border-top:1px solid #eef2f6}.obsMetricRow:first-child{border-top:0}.obsMetricRow>div{padding:6px 7px;border-left:1px solid #eef2f6;min-width:0;overflow-wrap:anywhere}.obsMetricRow>div:first-child{border-left:0}.obsMetricName{font-weight:900;color:#27364a}.obsMetricRow .forecast{background:#f4f7ff}.obsMetricRow .actual{background:#f1fbf5}.obsMetricRow b{margin-left:3px}
