...
function renderBoard(){const host=ensure();if(!host||typeof DATA==='undefined'||!DATA)return false;const tab=activeTab();if(tab==='official-macro'){host.style.display='none';return true}host.style.display='';const cfg=TAB_CONFIG[tab]||TAB_CONFIG.overview;if(cfg.special==='sector')renderSector();else if(cfg.special==='tripod')renderTripod();else if(cfg.special==='calendar')renderCalendarSummary();else renderStandard(tab,cfg);if(tab==='indices')setTimeout(renderIndexPanel,0);return true}
...
function ensureOfficialMacroRuntime(){
  ...
  const existing=document.querySelector('script[data-official-macro-runtime]');
  if(existing && typeof tries!=='undefined' && tries<30)return false;
...
window.__JJOONI_LATEST_BOARD={version:'2.4.2',contract:'GROUPED_MARKET_BOARD_WITH_INDEX_TAB_PERIOD_COMPARISON_AND_SOURCE_BASIS',tabs:Object.keys(TAB_CONFIG),comparisons:Object.keys(COMPARES)};
})();
