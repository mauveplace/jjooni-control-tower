...
const S={state:'ACTIVE',version:'31.6-public-daily-fallback',activations:0,market_augments:0,market_repairs:0,tripod_repairs:0};
...
function mergedMarket(){
 ...
 const publicDaily=U.market_context&&typeof U.market_context==='object'?U.market_context:{};
 const ci=canonical.items&&typeof canonical.items==='object'?canonical.items:{};
 const pi=publicDaily.items&&typeof publicDaily.items==='object'?publicDaily.items:{};
 const li=live.items&&typeof live.items==='object'?live.items:{};
 const items={};
 for(const key of new Set([...Object.keys(ci),...Object.keys(pi),...Object.keys(li)])){
   const safeFallback=mergeRecord(ci[key],pi[key]);
   items[key]=mergeRecord(safeFallback,li[key]);
 }
 const merged={...canonical,...publicDaily,...live,items};
 merged._ct_merge={canonical_items:Object.keys(ci).length,public_daily_items:Object.keys(pi).length,live_items:Object.keys(li).length,merged_items:Object.keys(items).length};
 return merged;
}
...
function sourceLabel(s){const x=String(s||'').toUpperCase();if(x.includes('KIS'))return'KIS';if(x.includes('YAHOO_PUBLIC_DAILY'))return'Yahoo 일봉';if(x.includes('CNN_FEAR_GREED_PUBLIC'))return'CNN 일봉';if(x.includes('CNN'))return'CNN 참고';if(x.includes('YAHOO'))return'Yahoo 참고';if(x==='UNAVAILABLE')return'확인 불가';return s||'—'}
...
document.addEventListener('jjooni:public-market-loaded',()=>setTimeout(()=>{augmentMarketCards();repairTripodSignal()},0));
