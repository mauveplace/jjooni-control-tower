...
function addAmountSort(root,metrics){
 ...
 let b=qs('[data-sort-amount-v5]',sorts);if(!b){b=document.createElement('button');b.type='button';b.className='ctTrChip';b.dataset.sortAmountV5='1';b.textContent='금액순';sorts.appendChild(b);b.onclick=e=>{e.stopPropagation();state.amountSort=true;apply()}}
 b.classList.toggle('on',state.amountSort);
 if(state.amountSort){qsa('[data-sort]',sorts).forEach(x=>x.classList.remove('on'));const cards=qsa('.ctTicker',root);cards.sort((a,b)=>z(metrics.get(b.dataset.ticker)?.amountScore)-z(metrics.get(a.dataset.ticker)?.amountScore)).forEach(x=>x.parentNode.appendChild(x))}
}
function annotateCards(root,metrics){
 qsa('.ctTicker[data-ticker]',root).forEach(card=>{
   ...
   const lab=moneyLabel(m);e.textContent=lab.text;e.className='ctTickerMoneyV5 '+lab.cls;e.title='금액순 기준: 평가손익 + 기회손실/회피손실의 절대 금액 합계';
 });
}
function setDefaultAll(root){
 if(state.defaultFilterApplied)return;
 const all=qs('[data-filter="all"]',root);if(!all)return;
 state.defaultFilterApplied=true;
 if(!all.classList.contains('on'))all.click();
}
function apply(){
