...
function mergePositions(live){
 ...
 ['ISA','PENSION','IRP'].forEach(id=>{const x=a[id]||{};(x.positions||[]).forEach(p=>{const k=sym(p.ticker),i=D.human.positions.findIndex(q=>String(q.account||q.account_type||'').toUpperCase()===id&&sym(q.ticker)===k),m={...(i>=0?D.human.positions[i]:{}),...p,account:id,account_type:id,record_type:String(p.record_type||'POSITION').toUpperCase(),current_price:z(p.current_price),market_value:z(p.market_value),avg_price:z(p.avg_price),price_source:p.price_source||'KIS_MARKET_QUOTE',data_state:p.data_state||'CURRENT'};if(i>=0)D.human.positions[i]=m;else D.human.positions.push(m)})});
